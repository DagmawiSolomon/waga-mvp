...
const TimelineItem = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { index?: number }>(
  ({ className, index = 0, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn("group relative pb-12", isEven ? "sm:pr-[calc(50%+2rem)]" : "sm:pl-[calc(50%+2rem)]", className)}
        {...props}
      />
    )
  },
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "after:absolute after:left-2 sm:after:left-1/2 after:box-content after:h-2 after:w-2 after:-translate-x-1/2 after:translate-y-1.5 after:rounded-full after:border-4 after:border-primary-foreground/95 after:bg-foreground",
        className,
      )}
      {...props}
    />
...
const TimelineDescription = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { index?: number }>(
  ({ className, children, index = 0, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn("text-muted-foreground text-sm sm:text-base sm:max-w-[70%] pt-1 pl-0", isEven ? "sm:text-right" : "sm:text-left", className)}
        {...props}
      >
        {children}
      </div>
